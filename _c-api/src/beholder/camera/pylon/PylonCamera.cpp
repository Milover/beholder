...
#include "beholder/camera/pylon/PylonCamera.h"
...
bool PylonCamera::triggerImpl(TriggerType typ) {
	switch (typ) {
...
bool PylonCamera::acquire(Milliseconds timeout) {
	if (!isAttached()) {
...
bool PylonCamera::cmdExecute(const std::string& cmd) noexcept {
	return cmdExecute(cmd.c_str());
}

bool PylonCamera::cmdExecute(const char* cmd) noexcept {
	try {
...
bool PylonCamera::cmdIsDone(const std::string& cmd) noexcept {
	return cmdIsDone(cmd.c_str());
}

bool PylonCamera::cmdIsDone(const char* cmd) noexcept {
	try {
...
std::optional<Image> PylonCamera::getImage() noexcept {
...
ParamList PylonCamera::getParams(ParamAccessMode mode) {
	ParamList params;
...
bool PylonCamera::isAcquiring() const noexcept { return cam_->IsGrabbing(); }

bool PylonCamera::init(Pylon::IPylonDevice* d) noexcept {
...
bool PylonCamera::isInitialized() const noexcept {
	return isAttached() && cam_->IsOpen();
}

bool PylonCamera::isAttached() const noexcept {
	return cam_->IsPylonDeviceAttached() && !cam_->IsCameraDeviceRemoved();
}

bool PylonCamera::setParams(const ParamList& params) noexcept {
	if (!isInitialized()) {
...
bool PylonCamera::startAcquisition(size_t nImages) noexcept {
...
void PylonCamera::stopAcquisition() noexcept { cam_->StopGrabbing(); }

bool PylonCamera::trigger(TriggerType typ) noexcept {
	try {
...
bool PylonCamera::waitAndTrigger(Milliseconds timeout,
